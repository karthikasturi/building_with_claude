...
class LoanApplicationRecord(BaseModel):
    ...
    TODO (Phase 2):
    - Fill in the correct types for each field (str, float, Literal, Optional, bool)
...
def build_policy_index(policy_path: str = "data/loan_processing_sop.md") -> object:
    """Chunk and embed the Apex Bank Credit Policy SOP.

    TODO (Phase 4):
    - Read the file at policy_path
    - Split into chunks of ~400 words with 50-word overlap (reference day3/rag_pipeline.py)
    ...
    - Build a VectorStore (copy the class from day3/rag_pipeline.py) and add all chunks
    - Return the populated VectorStore

    Hint: import openai inside this function (lazy import) so the file runs
    without an OpenAI key when only Phase 1–3 features are used.
    """
...
def retrieve_policy_context(query: str, store: object, top_k: int = TOP_K_CHUNKS) -> str:
    ...
    - Call store.search(query_embedding, top_k=top_k)
